...
#define MMC_SS_HIGH() PORTB |= _BV(SS_PIN)
#define MMC_SS_LOW() PORTB &= ~_BV(SS_PIN)

void SPI_Enable()   // TODO MISO and MOSI dir
{
  SPI_DDR &= ~_BV(MISO);
  SPI_DDR |= _BV(MOSI_PIN) | _BV(SCK_PIN) | _BV(SS_PIN);
  SPI_PORT |= _BV(MOSI_PIN) | _BV(SCK_PIN) | _BV(SS_PIN);
  SPCR = 0x50;
}
...
uint8_t SPI_ReceiveByte(uint8_t b)
{
    SPDR = b;
...
void SPI_Receive(uint8_t* buffer, int len)
{
    //  Unroll a bit
    SPDR = 0xFF;    // Byte 0        
    uint8_t i = 0xFF;
...
uint8_t MMC_Command2(uint8_t cmd, uint32_t param)
{
    uint8_t* b = (uint8_t*)&param;
    uint8_t d[7];
    d[0] = 0xFF;
    d[1] = cmd + 0x40;
    d[2] = b[3];
...
uint8_t MMC_Command(uint8_t cmd, uint32_t param)
{
    uint8_t* b = (uint8_t*)&param;
    uint8_t csum = 0xFF;
    if (cmd == 0)
...
uint8_t MMC_ReadOCR(uint8_t cmd, uint32_t param, uint8_t* ocr)
{
    uint8_t r = MMC_Command(cmd,param);
    uint8_t i = 4;
    while (i--)
...
uint8_t MMC_Release(uint8_t result)
{
    MMC_SS_HIGH();
